...
function polynomialRegression(points, degree) {
  ...
  // Constructing the Vandermonde matrix (X matrix) for the polynomial fit (degree 3)
  let X = [];
  for (let i = 0; i < points.length; i++) {
    X[i] = [];
    for (let j = 0; j <= degree; j++) {
      X[i][j] = Math.pow(xValues[i], degree - j); // For degree 3, this gives [1, x, x^2, x^3]
    }
  }
...
function generatePolynomialCurve(coefficients, points) {
  return points.map(point => {
    ...
    for (let i = 0; i < coefficients.length; i++) {
      y += coefficients[i] * Math.pow(point.x, coefficients.length - 1 - i); // Sum terms for y = a0 + a1*x + a2*x^2 + a3*x^3
    }
